...
「Rust にしたら速い」ではなく、「SQLite に置いたグラフを速く扱うには、どこを materialize し、どこを batch 化し、どこで Python object 化を避けるべきか」という話です。
...
RDB 的には foreign key の整数 ID で結びたくなるが、コードベースのグラフでは qualified name の方が扱いやすい場面が多い。
...
SQLite は in-process なのでネットワーク round trip はない。それでも SQL statement の発行、row materialization、Python object 化、query planner の起動はコストになる。frontier batching は、この固定費を訪問 node 数に比例させないための基本方針である。
...
この設計は Rust backend でも効く。PyO3 境界を file ごとに越えるのではなく、batch ごとに越える。SQLite transaction も Python/Rust boundary crossing も、どちらも回数を減らす。
...
Graph traversal は start node が決まっていないと始まらない。

AI agent から使う場合、最初の query は大体自然言語で来る。
...
なので、graph traversal の前に search が必要になる。
...
つまり、Python 実装が遅いときに最初に見るべきなのは言語ではない。SQL の打ち方、transaction の粒度、cache の寿命、derived data の持ち方である。
...
Rust 化でさらに速くできる可能性はある。ただし、それは writer 関数だけを置き換えることではなく、Python object を作る前の段階から境界を減らすことにある。
...
ここでいう `parser` は、Markdown や各言語の構文解析器を自前実装するという意味ではない。構文木を作る部分は Tree-sitter に任せる。Rust 側に寄せたいのは、Tree-sitter grammar のロード、対象 file の選別、AST traversal、dagayn 固有の node / edge 抽出、正規化、SQLite write までの一連のパイプラインである。

ここで grammar を差し替えると別の問題が起きる。Python 実装が `manji-0/tree-sitter-terraform` のような pinned grammar を使っているなら、Rust 側も同じ grammar と同じ query semantics に寄せる必要がある。`tree-sitter-hcl` のような近い別物に置き換えると、抽出できる node / edge が変わり、parity test で一致しなくなる。

つまり、Tree-sitter orchestration / extraction / normalization / writer / postprocess を同じ側に寄せる。PyO3 境界を越えるのは「build して」「summary を返して」くらいでよい。Python は MCP や CLI の interface layer に寄せる。
...
dagayn では、その最初の一歩として Markdown の抽出処理を Rust 側に寄せている。
...
Rust 化は、この延長線上にある。言語を変えるだけでは速くならない。Rust writer 単体では、release build でようやく Python writer とほぼ同等というところだった。

一方で、Tree-sitter orchestration、node / edge extraction、normalization、writer、postprocess をまとめて Rust core に寄せるなら、Python object 化、PyO3 marshalling、NetworkX 用の再構築、process 間 serialize といったコストを減らせる可能性がある。
