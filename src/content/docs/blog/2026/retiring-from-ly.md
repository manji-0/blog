...
https://manji0.hatenablog.com/entry/20190615/1560553200

入社した直後、部署内でSREチームが立ち上がり、当時急拡大していた社内クラウド（以後Verda）でパンクしかけていたデプロイシステムや監視システムを頑張って書き直していたらそのまま3年くらい経過してました。
...
当時はメンバーの全員が裁量労働であり、基本的に深夜対応をする個人的なインセンティブは、制度上存在しなかったので、大分頑張って上を説得して待機手当付きの制度を作ったことを覚えています。ちゃんと待機することにインセンティブのある制度設計を通せたのは我ながらいい仕事だったと思います。

---

LINEに入って2年ほどたったころ、たまたまマネージャー層の辞職が相次いだ時期にSREチームのマネージャーに任命されました。あまりポジティブな理由ではないですが、まあこうなったらやるしかないです。多分当時では社内でも最も低い年齢層でのマネージャー任命だったと思います。

さて、マネージャーになったタイミングではSREチームは合計で4名しかおらず、できることは限られていました。

前任のマネージャーはSREチームの前身であるOperation Unit..。つまるところ組織全体の雑用、調整役という重要であるがテーマを持ち辛い組織運営を継承しており、SREを導入するための基礎を整えるようなスタートラインまで手が回っていなかったという背景もあった。

まずは現状の組織名とは異なる期待などを覆せるような大玉を軸に据え、それはそれとして現状抱えていて放置できない仕事は粛々と進められるような組織設計をしなければと感じたことを覚えています。

広報や採用活動も大事です。対外発信やAtCoderコンテストのスポンサーなどいくつか施策を打ったり、チーム内での構造化面接の設計と導入を推進したりなど、人員不足を解決するためなんとか頑張ったり。

https://gihyo.jp/dev/serial/01/line2021/0008

https://atcoder.jp/contests/abc263

チームの成果としては、発足から1年弱かけて組織全体のメトリクス、ログのストレージとインターフェースを統一でき、それぞれのVerdaサービスが個別で運用していたパイプラインの多くの削減を達成しました。
...
これは頑張って本業の傍らで進めていた採用広報や対外発信の効果が出たものだと思います。綱渡りですが、なんとかやりきれましたね。

---

Verdaの独立したドキュメントサイトを社内に立ち上げたのも確かマネージャー期間だった記憶があります。当時はVerdaのドキュメントが他のあらゆる社内ドキュメントと一緒にオンプレのConfluenceに蓄積されていた。

古い情報が消されず山積みになっていたり、執筆のレビュープロセスが整備されていなかったり、そもそものConfluenceの検索機能がとても使い辛かったりしていた。これらを解消したいという思いから企画したことを覚えています。
...
新しいOpenStackのバージョンを使うということでPython2を基本に書かれた既存のデプロイコードがほとんど使えないということが明らかになり、Python3ベースのデプロイコードを2か月で作ることを強行することになり、入籍してから同居開始までの期間はあんまり寝ずに働いていたことを覚えています。

恐らく、k8s manifest含めて2か月で0から3万行も手書きしたことがある人は社内でほとんどいないでしょうし、今後もそんなに書くことはもうないだろうなと思います。
...
* マネージャー任命の頃は確かもうフルリモートでした。コロナ真っ最中じゃなかったかな。
  * 前任のマネージャーは優れた人格者であると同時に趣味の合う人で、よく喫煙所のタイミングが一緒になり仕事や組織の議論を長々としていました。リモートワークによってそのようなことは無くなり、広々としたミライナタワーの喫煙所もオフィス撤退で使えなくなり、寂しい思いをしましたね。
  * 睡眠が崩れた原因の1つが仕事部屋と寝室が同じ部屋だったことで、それについては翌年に3LDKのマンションを買って根本解決しました。
  * リモート移行期は混乱が大きく、ルール設定前にディスプレイを規程枚数以上自宅用に貸与できたりしていました（施行時期の問題でありルール違反とはいえないが。..）。退社までに返却しなきゃいけないのですが、梱包が大変です。
...
* 新クラウド基盤について取り組んだ大きな仕事の1つが物理故障の報告・対応フローの自動化で、連絡や復旧の状態を完全にステートマシンと捉えてk8s operator+CRDの状態遷移として手順を自動実行するようにしています。
  * 最初に書いた殴り書きコードは運用のための引き継ぎの段階で他メンバーにNGを食らい、クリーンな構成を1から勉強してAIでほぼ100%リファクタしました。AIのパワーを感じたのはその頃からです。
* 次の職場は入社後にまた記事を書こうと思いますが、裁量労働+フルリモートです。年収はちょっと上がる予定。
  * 去年の源泉徴収は育休2か月分を差し引いて1100万くらいでした。入社時が750万だったので、いいペースで上がってたと思います。
* 第一子が今6か月で、夜泣きもかなりあります。21時~2時くらいまで僕が面倒を見てそこから寝るので、非常に朝が弱い僕には午前中出社はかなり厳しい。おそらく第二子以降も似たようなリズムになるでしょうし。
