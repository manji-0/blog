...
さて、振り返ったところで改善する方法を考える。幸い LINE は採用現場の権限がかなり広く、オリジナルのフローを作って運用するくらいのことはチームマネージャーが自由にやって OK だったので、ゴリゴリ弄ることにした。
...
LinkedIn の行動面接 質問集を参考にした。採用基準はその分類をベースに、分類とレベル分けをして構造化した。
...
あとJDを是非読んでほしい。頑張って書いたけどフィードバックが少なくてこっからどうすればいいか分からんので。分かりにくかったら直すので。コメントは Twitter でもメールでも OK です。
