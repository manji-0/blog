...
**免責事項:**
* 具体的な採用基準については書かないどっかから怒られそうなので
...
..。いや難しいというか無理だよ。面接は1時間しかないのに最後まで進められるわけがない。少なくとも僕の腕では無理。過去にこのやり方で今のメンバーが集まったのは奇跡だと思う。今までの担当者は面接のプロ集団だったに違いない。

さて、振り返ったところで改善する方法を考える。幸いLINEは採用現場の権限がかなり広く、オリジナルのフローを作って運用するくらいのことはチームマネージャーが自由にやってOKだったので、ゴリゴリ弄ることにした。
...
まず参考にしたのが、LINE社員の行動指針であるLINE STYLE。
...
LinkedInの行動面接質問集を参考にした。採用基準はその分類をベースに、分類とレベル分けをして構造化した。
...
2. 書類選考（どういう観点でチェックするか、逆に判断へ使ってはいけない情報は何かを明文化した）
...
あとJDを是非読んでほしい。頑張って書いたけどフィードバックが少なく、ここからどうすればよいか見当がつかない。分かりにくかったら直すので。コメントはTwitterやメールでOKです。
