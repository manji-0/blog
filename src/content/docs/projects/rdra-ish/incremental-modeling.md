...
|---|---|---|---|
| **0** | スコープ | `business`、候補 `buc` | スライスの切り方 |
| **1** | BUC 骨格 | `actor`、`usecase`、`performs`、`contains` | アクターとUCがBUCに束ねられているか |
| **2** | データ接点 | 粗い `entity`、CRUD 述語 | UCごとにCRUDが説明可能か（`csv --kind matrix`） |
| **3** | 相互作用境界 | `screen`、`api`、`displays`、`invokes`、権限・媒体制約 | 画面/API/権限のギャップ（`csv --kind actor-permission-audit`） |
| **4** | エンティティ構造 | カラム詳細、`relate(..., N:1)`、カーディナリティ | ER整合、外部キー、所有関係 |
| **5** | ライフサイクル | Enum 列、`event`、`transitions(Entity.col, …)`、`raises`、`sets`（単独 `state` 宣言は任意） | 到達不能状態、未消費イベント（`states`, `diagram --kind event-flow`） |
| **6** | ルール | `forbidden` / `invariant`（マルチエンティティ可）、`when(...).none/has`、必要なら `property` + TLC | `forbidden` / `invariant`（`states`）。比較・時間性質は [形式検証](/projects/rdra-ish/formal-verification/) |
