...
| **4** | エンティティ構造 | カラム詳細、`relate(..., N:1)`、カーディナリティ |
| **5** | ライフサイクル | Enum 列、`event`、`transitions(Entity.col, …)`、`raises`、`sets`（単独 `state` 宣言は任意） |
| **6** | ルール | `forbidden` / `invariant`（マルチエンティティ可）、`when(...).none/has`、必要なら `property` + TLC |
...
| 6 | `forbidden` / `invariant` 違反（`states`）。比較・時間性質は [形式検証](/projects/rdra-ish/formal-verification/) |
