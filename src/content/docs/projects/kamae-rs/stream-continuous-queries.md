...
- プロジェクションがイベントIDや `(aggregate_id, sequence)` で冪等か。
- リード側ストリームが集約遷移や権威ある状態の永続化をしていないかも見る。
- 再起動後に再開できる永続カーソルはあるか。
- 無制限バッファやコンシューマ脱落後も読み続けるストリームはないか。
- 手書きの `loop { sleep; query }` より型付き `Stream<Item = Result<_, _>>` ポートの方が明確でないか。
- 未対応イベント版でパニックや黙殺していないか（[サービス境界](/projects/kamae-rs/service-boundaries/)）。
