...
`Cargo.toml` に応じて、必要なときだけ [クレートガイド](/projects/kamae-rs/crate-guides/) を見てください。エラーは `thiserror` / `anyhow`、シリアライズは `serde`、検証やnewtypeは `validator` / `garde` / `nutype` です。シークレットは `secrecy`、観測は `tracing` / `metrics`、テストは `proptest` あたりがガイド付きです。
