...
破壊的なフィールド改名や `schema_version` 欠落、未知イベント版でのパニックはないか。冪等キーなしに副作用を適用するコンシューマはないか（[永続化、集約、イベント](/projects/kamae-rs/persistence-events/)）。protobuf / JSON / キューペイロードを `TryFrom` なしにドメインへ渡していないかも見る。リトライやブレーカがドメイン内になく、`tonic` / `prost` 生成型がドメインクレートに漏れていないか。入口の `correlation_id` やトレースは外向き呼び出しへ伝播しているか。
