...
- 公開の `Result` 関数が重要なエラーバリアントを隠し、本番パニックに `# Panics`、`unsafe` に `# Safety` がない箇所はないか。
- 例がプライベート近道や無説明のunwrap、PII漏洩、不可能な遷移を示していないかも見る。
- 公開newtype・コンストラクタ・遷移・リポジトリトレイトに不変条件や単位が書かれているか。
- 壊れたintra-docリンクや古いdoctestはなく、公開ライブラリなら `rustdoc::broken_intra_doc_links` を検討できるか。
