...
触ったクレート向けにパッケージ検証、フォーマット、lint、テストを走らせる短いローカルコマンド一覧またはスクリプトを提案する。正規コマンドは [品質ゲート](/projects/kamae-rs/quality-gates/)、推奨の高速パスとフルpre-pushループは [開発環境](/projects/kamae-rs/dev-environment/) を照合する。
