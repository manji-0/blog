...
- publicフィールドや生リテラルで無効状態を作るテストになっていないか（マイグレーション互換・破損行・`trybuild` などは除く）。
- クロスフィールド不変条件、非法遷移、DTO変換失敗、マスキングされたログ、楽観ロックや重複コマンドのカバレッジはあるか。
- 例表がなく公開コンストラクタを使える値オブジェクトではプロパティテストを足せないか（[プロパティベーステスト](/projects/kamae-rs/property-based-tests/)）。
- コンパイル時状態安全性が中核なら `trybuild` も検討する。
