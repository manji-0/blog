...
| 両方変え、片方失敗でもう片方もロールバック必須 | 単一ユースケース、明示順序、saga（補償付きの複数ステップ）/outbox（状態と配信行の同TX保存）、または対応DBなら1トランザクション |
| 結果整合でよい（すぐ揃わなくてよい） | ドメインイベント + 下流consumer |
...
- 無関係な呼び出し元から状態保存・イベント発行を担わせていないか。
- version/CASなしの競合書き込みや、集約ルートを迂回する変更はないか。
- 楽観で足りるのに広い悲観ロックを `.await` またぎで取っていないかも見る。
- 冪等キーなしの二重適用は境界で塞がれているか。
