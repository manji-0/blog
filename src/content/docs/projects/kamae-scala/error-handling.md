...
- エラーテキストにメール・電話・トークン・生ボディが入っていないか。
- ドメインやユースケースで `throw` / `???` / unsafe `.get` が常態化していないか。
- 排他リソースを `F` のbind / `await` またいで持っていないか。
- I/O付きドメイン遷移やインフラエラーの素通しがないか。
- DB / HTTP失敗を公開APIへそのまま出していないか。
- ドメインエラーが `Throwable` / `String` / catch-allになっていないか。
