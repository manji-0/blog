...
- マスキングは[PII 保護](/projects/kamae-scala/pii-protection/)も参照する。
- 生の機密値がログ・スパン・メトリクス・エラー表示に出ていないか。
- フィールド名の仮定だけでIDをログしていないか（レビュー済みの不透明サロゲート集約IDは除く）。
- 意味のないメッセージ、ドメイン文脈や遷移の欠如、エラーの重複ログ、高カーディナリティラベル、非構造化`println`がないかも見る。
