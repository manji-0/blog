...
`build.sbt` に応じて、必要なときだけ [ライブラリガイド](/projects/kamae-scala/library-guides/) を見てください。エフェクトは `cats` / `zio`、JSONは `circe`、HTTPは `http4s` / `sttp`、検証は `refined`、永続化は `doobie`、ストリームは `fs2`、設定は `pureconfig`、テストは `scalacheck` あたりがガイド付きです。
