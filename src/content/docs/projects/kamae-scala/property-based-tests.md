...
プロパティテストは入力を広げるが、アサーションの強さまでは証明しない。高リスクモジュールでプロパティと例が緑になったあと、静かなギャップを探すなら [ミューテーションテスト](/projects/kamae-scala/mutation-testing/)（Stryker）を検討する。
...
`apply`や`Either`検証companionではなく生リテラルでドメインを組むScalaCheck戦略がないか。ライブI/Oに当たる`forAll`や法則なしの`isRight`/`isLeft`のみ、曖昧な前提の扱いがないかも見る。非法遷移で特定エラーまで見ているか。微妙なバグの最小反例にはseedコメントや`propWithSeed`も検討する。
