...
- `apply`や`Either`検証companionではなく生リテラルでドメインを組むScalaCheck戦略がないか。
- ライブI/Oに当たる`forAll`や法則なしの`isRight`/`isLeft`のみ、曖昧な前提の扱いがないかも見る。
- 非法遷移で特定エラーまで見ているか。
- 微妙なバグの最小反例にはseedコメントや`propWithSeed`も検討する。
