...
型設計は [ドメインモデリング](/projects/kamae-scala/domain-modeling/)、JNIの隔離は [JNI / ネイティブ境界](/projects/kamae-scala/jni-native-boundaries/)、チェックの自動化は [品質ゲート](/projects/kamae-scala/quality-gates/) と整合させる。
...
公開libraryをCIでpublishするプロジェクトでは `sbt doc` を品質ゲートに含める（[品質ゲート](/projects/kamae-scala/quality-gates/) 参照）。

レビューでは、契約欠如の公開API、隠されたerrorケース、不安全なScaladoc例、壊れた `[[Type]]` リンクを指摘する。

## レビューで見るところ

ドメイン`Either`を返す公開関数で重要なerrorバリアントが隠れていないか。公開ドメインAPIに不変条件・単位・ライフサイクル・副作用の契約があるか。例がプライベート近道や`.get`、PII漏洩、不可能な遷移を示していないかも見る。壊れた`[[Type]]`リンクや現行挙動と矛盾するドキュメントがないか確認する。
