...
スキルテンプレートから **アプリケーションリポジトリ** を立ち上げる担当者向けの手順書である。ツールチェーンとテンプレートの適用が揃っていないと、以降のドメイン規約をローカルで再現できず、CI上だけが正しく見える状態になりやすい。

| やりたいこと | 読むページ |
| --- | --- |
| アプリの初回ブートストラップ | このページ |
| テンプレート適用後のローカル検証 | [ローカル検証セットアップ](/projects/kamae-py/local-validation/) |
| 日常のチェックコマンド | [品質ゲート](/projects/kamae-py/quality-gates/) |
| GitHub Actions | [CI セットアップ](/projects/kamae-py/ci-setup/) |

スキルパッケージ本体（kamae-pyリポジトリ）のコントリビューション手順も、後半のテンプレート節と重なる。アプリ開発だけならブートストラップと品質ゲートで足りる。
...
pydantic-settingsで `.env` を使う（[境界防御](/projects/kamae-py/boundary-defense/#environment-and-cli-boundaries) を参照）。`.env` を `.gitignore` に追加する。
...
ブートストラップ後、[品質ゲート](/projects/kamae-py/quality-gates/) のベースラインコマンドを実行する。スキル/プラグインリポジトリではさらに `uv run python scripts/validate_package.py` を実行する。
...
[品質ゲート](/projects/kamae-py/quality-gates/) のベースラインコマンドを実行する。このリポジトリではさらに次を実行する：
...
コミットされた `.env` や例に実シークレット・生PIIがないか（[PII と観測経路の保護](/projects/kamae-py/pii-protection/)）。`domain` がFastAPI・Django・SQLAlchemy・boto3をimportしていないかも見る。ドメイン/ユースケーステストがHTTPやライブDBではなくフェイクで動き、フィクスチャがコンストラクタ経由か（[テストデータ](/projects/kamae-py/test-data/)）。CIと整合するローカルチェック一覧はあるか（[CI セットアップ](/projects/kamae-py/ci-setup/)）。
