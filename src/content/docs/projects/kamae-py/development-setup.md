...
- コミットされた `.env` や例に実シークレット・生PIIがないか（[PII と観測経路の保護](/projects/kamae-py/pii-protection/)）。
- `domain` がFastAPI・Django・SQLAlchemy・boto3をimportしていないかも見る。
- ドメイン/ユースケーステストがHTTPやライブDBではなくフェイクで動き、フィクスチャがコンストラクタ経由か（[テストデータ](/projects/kamae-py/test-data/)）。
- CIと整合するローカルチェック一覧はあるか（[CI セットアップ](/projects/kamae-py/ci-setup/)）。
