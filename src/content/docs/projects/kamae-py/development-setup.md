...
日常のチェックコマンドは [品質ゲート](/projects/kamae-py/quality-gates/)、GitHub Actionsの配線は [CI セットアップ](/projects/kamae-py/ci-setup/) を参照する。
...
[PII と観測経路の保護](/projects/kamae-py/pii-protection/) と照合する。コミットされた `.env`、例の実認証情報、デバッグ用に生PIIをログするよう促すローカルセットアップ文書を指摘する。
...
[テストデータ](/projects/kamae-py/test-data/) と照合する。ドメイン/ユースケーステストで生dict、`model_construct`、ORM行により不変条件を迂回しているテストヘルパーを指摘する。

### 文書化されたローカルチェックループがあるか — Low

[CI セットアップ](/projects/kamae-py/ci-setup/) と整合するファストパスとフルpre-pushコマンド一覧なしにKamae規約を採用するプロジェクトを指摘する。
