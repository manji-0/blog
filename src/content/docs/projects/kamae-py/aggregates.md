...
1. **ドメインイベント + handler** で第二集約を更新（**結果整合**: すぐには揃わず、後続処理で追いつく）
2. **プロセスマネージャ / saga** と補償ステップ（複数ステップの進行と失敗時の巻き戻しを明示する）
3. 真の不変条件が原子性を要求するなら**単一集約への再設計**

状態変更と一緒に配信用の行を同じトランザクションで書く仕組みを **アウトボックス（outbox）** と呼ぶ。詳細は [永続化、集約、イベント](/projects/kamae-py/persistence-events/) へ。
...
| 層 | 所有するもの |
| --- | --- |
| **ユースケース** | 作業単位（load → authorize → transition → save）。どこまでを1コマンドの境界にするか |
| **リポジトリアダプタ** | `save(...)` 内のBEGIN/COMMIT/ROLLBACK。状態とoutboxを原子的に書くこと |

ユースケースが「境界」を決め、アダプタがDBトランザクションを実装する。ドメインコードはBEGIN/COMMITしない。

portメソッドに原子性の期待を文書化する。パラメータは [永続化、集約、イベント](/projects/kamae-py/persistence-events/) の正規portに合わせる：
...
- 1ユースケースが1トランザクションで2ルートを更新していないか
- 状態とイベントがoutbox等でアトミックに永続化されているか
- 集約ルートを迂回する直接更新はないか
- 楽観的版管理なしの競合書き込みはないか
- 悲観的ロックがドメイン遷移に漏れていないか
- DB制約と狭いリポジトリ `Protocol` がアプリケーション検査を補完しているか
