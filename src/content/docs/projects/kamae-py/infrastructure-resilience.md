...
副作用のあるリトライ・アウトボックス・外部呼び出しに冪等キーや重複排除があるか（[永続化、集約、イベント](/projects/kamae-py/persistence-events/)）。タイムアウトとサーキットブレーカーがアダプターで明示されているかも見る。リトライやブレーカーがドメイン遷移に入り込んでいないか、バリデーション拒否まで広い例外で隠していないかも確認する。
