...
- リード側ストリームが集約遷移や権威ある状態の永続化をしていないか。
- 再起動後に再開できる永続カーソルはあるか。
- 無制限バッファやコンシューマ脱落後も読み続けるストリームはないか。
- 手書きの `while True: sleep; poll` より型付き `AsyncIterator` portの方が明確でないか。
- 未対応イベント版でパニックや黙殺していないか（[サービス境界](/projects/kamae-py/service-boundaries/)）。
