...
- 公開関数の重要エラーバリアントや例外のキャッチ層、ネイティブラッパーの呼び出し元義務が文書化されているか。
- 例が `model_construct` やDTO迂回、PII漏洩、不可能遷移を示していないかも見る。
- 公開ドメインAPIの不変条件・単位・副作用がdocstringにあり、古い例や型名が残っていないかも確認する。
- 公開ライブラリなら壊れた例を検知する手段があるかも見る。
