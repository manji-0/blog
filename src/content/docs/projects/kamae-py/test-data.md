...
遷移とイベントの期待は [状態遷移](/projects/kamae-py/state-transitions/)、ログとIDのアサーションは [PII と観測経路の保護](/projects/kamae-py/pii-protection/) と [ロギングとメトリクス](/projects/kamae-py/logging-metrics/) に従う。
...
識別子方針は [PII と観測経路の保護](/projects/kamae-py/pii-protection/) のティア別ルールを検証する：
...
無効状態を `model_construct` や生dictで作るテストがないか（移行・破損行・ネガティブパスは除く）。クロスフィールド不変条件、拒否遷移、DTO変換、マスク付きログの境界失敗はカバーされているかも見る（[PII と観測経路の保護](/projects/kamae-py/pii-protection/)）。DB制約・楽観ロック・リトライ・アウトボックスのエッジはテストされているか。公開コンストラクタがある値オブジェクトや遷移法則にHypothesisや `assert_never` を足せるかも確認する。
