...
| **C — 相関** | 1 ワークフローまたは集約用のシステム生成 ID | 構造化属性で可 | 通常不可。不透明エラーコードを使う | 決して | 決して |
...
2つのTier D IDが同じ行で再識別を容易にするとき（例： 同じ行の `passenger_id` + `driver_id`）、タスクに必要な最小集合をログする。十分なら集約 `request_id` を優先する。

### ティア E — 語彙（どこでも安全）

閉じた集合からの低カーディナリティ値：

- `kind`（現在の集約状態）、`source_kind`、`target_kind`
...
メール・電話・トークンなどを素の `str` / `bytes` で運び、`SecretStr` やマスクラッパーなしで `repr`・ログ・メトリクス・エラーに出していないか。任意ドメインオブジェクトを許可リストなしでオブザーバビリティへ渡していないかも見る。根拠なく人物紐づきIDをログしていないか、生値を返す広いゲッターになっていないかも確認する。
