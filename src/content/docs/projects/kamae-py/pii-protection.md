...
- メール・電話・トークンなどを素の `str` / `bytes` で運び、`SecretStr` やマスクラッパーなしで `repr`・ログ・メトリクス・エラーに出していないか。
- 任意ドメインオブジェクトを許可リストなしでオブザーバビリティへ渡していないかも見る。
- 根拠なく人物紐づきIDをログしていないか、生値を返す広いゲッターになっていないかも確認する。
