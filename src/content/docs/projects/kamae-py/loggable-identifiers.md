...
メッセージ文字列やメトリクスラベルに `request_id` や `user_id` が補間されていないか。`model_dump()` やトークン平文が `extra` に入っていないか。ティアDを信頼ゾーン外へ出すときにハッシュやトークン化しているか。許可リストをバイパスする生dictログはないか。2つのティアDを同一行に載せる必要がある根拠はあるか。
