...
外部HTTP、キュー、ファイル入力で検証をスキップするために `model_construct` を使わない。完全な方針は [境界防御](/docs/kamae-py/boundary-defense/) と [unsafe 境界](/docs/kamae-py/unsafe-boundaries/) を読む。
...
3. CPUバウンドバッチは `asyncio.to_thread` またはワーカープールへオフロードを検討。[並行性と非同期](/docs/kamae-py/concurrency/) を参照。

## 不変条件を迂回せずに作業を減らす

1. **ユースケースごとにモデルを分割する。** リストビューは完全アグリゲート共用体を必要としない。リポジトリポートで狭い読み取りDTOを使う。
2. **純粋遷移は安価に保つ。** 遷移関数はすでに検証済みドメイン状態を受け取る。すべてのフィールドでJSONを再パースしたりPydanticを再実行したりしない。
3. **プロセス内専用ヘルパーには dataclass を優先する。** 選択表は [ドメインモデリング](/docs/kamae-py/domain-modeling/) を参照。明示的マッパーなしに同じ概念をPydanticとdataclassの両方で重複しない。
...
外部システムの生dictをキャッシュし、キャッシュミス時の再検証なしにドメインオブジェクトとして扱わない。無効化はアグリゲートバージョンまたはTTL方針に結び付ける。
...
CPUバウンド検証または変換をasyncioイベントループから外すべきときは [並行性と非同期](/docs/kamae-py/concurrency/) を読む。

## レビュー観点

### `model_construct` は信頼できる内部経路に限定されているか — High

完全バリデーションが必要な境界データ、ORM行、部分的に信頼できるdictへの `model_construct` を指摘する。
...
プロファイリングやスループット要件がトレードオフを正当化するときだけmsgspecや狭いDTOアダプターを提案する。
