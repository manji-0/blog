...
> **関連:** [`domain-modeling.md`](/docs/kamae-py/domain-modeling/)、[`boundary-defense.md`](/docs/kamae-py/boundary-defense/)、[`unsafe-boundaries.md`](/docs/kamae-py/unsafe-boundaries/)。
...
| ロギング / メトリクス | 大きな状態への `model_dump` | 識別子と `kind` のみログ。[`pii-protection.md`](/docs/kamae-py/pii-protection/) を参照 |
...
外部 HTTP、キュー、ファイル入力で検証をスキップするために `model_construct` を使わない。完全な方針は [`boundary-defense.md`](/docs/kamae-py/boundary-defense/) と [`unsafe-boundaries.md`](/docs/kamae-py/unsafe-boundaries/) を読む。
...
3. CPU バウンドバッチは `asyncio.to_thread` またはワーカープールへオフロードを検討。[`concurrency.md`](/docs/kamae-py/concurrency/) を参照。
...
3. **プロセス内専用ヘルパーには dataclass を優先する。** 選択表は [`domain-modeling.md`](/docs/kamae-py/domain-modeling/) を参照。明示的マッパーなしに同じ概念を Pydantic と dataclass の両方で重複しない。
...
CPU バウンド検証または変換を asyncio イベントループから外すべきときは [`concurrency.md`](/docs/kamae-py/concurrency/) を読む。
