...
> **関連:** [`state-transitions.md`](/docs/kamae-py/state-transitions/)、[`infrastructure-resilience.md`](/docs/kamae-py/infrastructure-resilience/)、[`pii-protection.md`](/docs/kamae-py/pii-protection/)。
...
アダプターでのリトライ、タイムアウト、サーキットブレーカーの配置については [`infrastructure-resilience.md`](/docs/kamae-py/infrastructure-resilience/) を読む。
...
長いモナドチェーンより読みやすい早期リターンを優先する。[`state-transitions.md`](/docs/kamae-py/state-transitions/#keep-use-cases-thin) の**正規**ユースケースから始め、`save_en_route` 周辺に永続化エラーマッピングを追加する:
...
[`pii-protection.md`](/docs/kamae-py/pii-protection/) と突き合わせる。メール、電話、トークン、生 SQL/HTTP 本文を埋め込むエラーテキストを指摘する。
...
[`concurrency.md`](/docs/kamae-py/concurrency/) と突き合わせる。
