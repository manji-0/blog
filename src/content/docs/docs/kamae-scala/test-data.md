...
helperは `src/test/scala/.../support/` やモジュールローカル `test` パッケージで共有する。fake portパターンは [開発環境](/docs/kamae-scala/dev-environment/) を参照。
...
redacted log、安全errorメッセージ、安全metrics label、敏感データがあるときのresponse DTOシリアライズをobservabilityテストでassertする。識別子ポリシーは [ロギングとメトリクス](/docs/kamae-scala/logging-metrics/) のtierルールに従う。
...
多入力で成り立つ不変条件にはScalaCheck（[プロパティベーステスト](/docs/kamae-scala/property-based-tests/) 参照）。遷移が純関数で不変条件が明示的なKamae Scalaに合う。

PR前に [品質ゲート](/docs/kamae-scala/quality-gates/) のテストコマンドを実行する。
