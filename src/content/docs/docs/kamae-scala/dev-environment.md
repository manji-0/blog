...
Kamaeスキルに従う**アプリケーション sbt プロジェクト**をローカルで立ち上げる手順である。スキルパッケージ本体の編集は [スキルリポジトリの開発](/docs/kamae-scala/development-setup/) を読む。

日常のチェックは [品質ゲート](/docs/kamae-scala/quality-gates/)、フィクスチャの組み立ては [テストデータ](/docs/kamae-scala/test-data/)、CIへの反映は [CI セットアップ](/docs/kamae-scala/ci-setup/) を参照する。
...
- ハンドラと `main` がアダプターを配線する。ユースケースはポートtraitのみに依存する（[アプリケーション配線](/docs/kamae-scala/application-wiring/)）。
...
テスト用composition rootにフェイクを注入する。本番と同じコンストラクタでフィクスチャを構築する（[テストデータ](/docs/kamae-scala/test-data/)）。
...
[品質ゲート](/docs/kamae-scala/quality-gates/) と [CI セットアップ](/docs/kamae-scala/ci-setup/) に合わせる。編集中は高速パス、PR前はフルパスを使う。
...
probe出力はreview leadとして扱い、自動失敗にはしない。初回ブートストラップは [ローカル検証](/docs/kamae-scala/local-validation/) を参照する。
