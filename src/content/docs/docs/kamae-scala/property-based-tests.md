...
`org.scalacheck` がclasspathにあるとき [ライブラリガイド（scalacheck）](/docs/kamae-scala/library-guides/#scalacheck) を読む。
...
読みやすいシナリオはexampleベース、`compileErrors` テストは型安全性約束（[テストデータ](/docs/kamae-scala/test-data/) 参照）。
