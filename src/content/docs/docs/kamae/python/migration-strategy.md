...
> **関連:** [`boundary-defense.md`](/docs/kamae/python/boundary-defense/)、[`orm-adapters.md`](/docs/kamae/python/orm-adapters/)、[`persistence-events.md`](/docs/kamae/python/persistence-events/)。
...
サービスメソッドをポートを受け取る非同期関数にする。完全実装: [`state-transitions.md`](/docs/kamae/python/state-transitions/#keep-use-cases-thin)。
...
`mapped_column` エンティティ、行 DTO、`domain_from_row_dto` マッパー、Django `select_for_update` 書き込みパターンは [`orm-adapters.md`](/docs/kamae/python/orm-adapters/) を読む。
...
- 可観測性が [`pii-protection.md`](/docs/kamae/python/pii-protection/) に従う

最初に移行するワークフローの選択は [`persistence-events.md`](/docs/kamae/python/persistence-events/) と [`application-wiring.md`](/docs/kamae/python/application-wiring/) を読む。
...
[`pii-protection.md`](/docs/kamae/python/pii-protection/) と [`persistence-events.md`](/docs/kamae/python/persistence-events/) と突き合わせる。
