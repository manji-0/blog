...
> **関連:** [`domain-modeling.md`](/docs/kamae/python/domain-modeling/)、[`boundary-defense.md`](/docs/kamae/python/boundary-defense/)、[`unsafe-boundaries.md`](/docs/kamae/python/unsafe-boundaries/)。
...
| ロギング / メトリクス | 大きな状態への `model_dump` | 識別子と `kind` のみログ。[`pii-protection.md`](/docs/kamae/python/pii-protection/) を参照 |
| テスト | 同一フィクスチャの再検証 | コンストラクタまたはモジュールごとに 1 回キャッシュしたアダプターでフィクスチャ構築 |

`TypeAdapter` インスタンスはモジュールレベル定数であるべきである。Pydantic はバリデータをキャッシュする。リクエストごとに新しいアダプターを構築するとスキーマコンパイル作業を繰り返す。
...
外部 HTTP、キュー、ファイル入力で検証をスキップするために `model_construct` を使わない。完全な方針は [`boundary-defense.md`](/docs/kamae/python/boundary-defense/) と [`unsafe-boundaries.md`](/docs/kamae/python/unsafe-boundaries/) を読む。
...
3. CPU バウンドバッチは `asyncio.to_thread` またはワーカープールへオフロードを検討。[`concurrency.md`](/docs/kamae/python/concurrency/) を参照。
...
3. **プロセス内専用ヘルパーには dataclass を優先する。** 選択表は [`domain-modeling.md`](/docs/kamae/python/domain-modeling/) を参照。明示的マッパーなしに同じ概念を Pydantic と dataclass の両方で重複しない。
...
CPU バウンド検証または変換を asyncio イベントループから外すべきときは [`concurrency.md`](/docs/kamae/python/concurrency/) を読む。
