...
> **いつ読むか:** ユースケースの失敗のモデル化、HTTP レスポンスへのエラーマッピング、非同期 `Result` フロー、または例外を投げるべきかの判断をするときに読む。
> **関連:** [`state-transitions.md`](/docs/kamae/python/state-transitions/)、[`infrastructure-resilience.md`](/docs/kamae/python/infrastructure-resilience/)、[`pii-protection.md`](/docs/kamae/python/pii-protection/)。

## 期待される失敗は明示的に保つ

ユースケースの失敗は操作に固有であるべきである。すべてのビジネスパスに catch-all の `AppError` を使ってはならない。
...
アダプターでのリトライ、タイムアウト、サーキットブレーカーの配置については [`infrastructure-resilience.md`](/docs/kamae/python/infrastructure-resilience/) を読む。
...
長いモナドチェーンより読みやすい早期リターンを優先する。[`state-transitions.md`](/docs/kamae/python/state-transitions/#keep-use-cases-thin) の**正規**ユースケースから始め、`save_en_route` 周辺に永続化エラーマッピングを追加する:
...
呼び出し元が分岐する必要があるのに、ドメインコンストラクタやユースケースから `Exception`、裸の `ValueError`、`RuntimeError`、不透明な文字列エラーを返している箇所を指摘する。
...
[`concurrency.md`](/docs/kamae/python/concurrency/) と突き合わせる。
...
[`pii-protection.md`](/docs/kamae/python/pii-protection/) と突き合わせる。メール、電話、トークン、生 SQL/HTTP 本文を埋め込むエラーテキストを指摘する。
