...
> **いつ読むか:** CPU バウンドのドメイン作業、GIL、`ProcessPoolExecutor`、または asyncio イベントループのブロックが懸念されるとき。
> **関連:** [`application-wiring.md`](/docs/kamae/python/references/application-wiring/)、[`state-transitions.md`](/docs/kamae/python/references/state-transitions/)、[`infrastructure-resilience.md`](/docs/kamae/python/references/infrastructure-resilience/)。
...
完全なユースケースは [`state-transitions.md`](/docs/kamae/python/references/state-transitions/#keep-use-cases-thin) を参照。
...
ドメインモジュールは **asyncio なしでインポート可能**であるべき。ルール:
...
ポート配線は [`application-wiring.md`](/docs/kamae/python/references/application-wiring/)、遅いワーカー周りのタイムアウトとリトライは [`infrastructure-resilience.md`](/docs/kamae/python/references/infrastructure-resilience/) を読む。
