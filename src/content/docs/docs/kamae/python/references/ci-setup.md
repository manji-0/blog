...
> **対象読者:** スキルを自リポジトリにインストールするプロジェクト。`kamae-py` スキルリポジトリ自体については [`development-setup.md`](/docs/kamae/python/references/development-setup/) を読む。
> **いつ読むか:** GitHub Actions の作成・更新、ブランチ保護の指針、リポジトリ検証ジョブ。
> **関連:** [`quality-gates.md`](/docs/kamae/python/references/quality-gates/)（CI が実行すべきチェック）、[`local-validation.md`](/docs/kamae/python/references/local-validation/)。

## デフォルトの GitHub Actions ワークフロー

CI は [`quality-gates.md`](/docs/kamae/python/references/quality-gates/) と同じチェックを実行すべきである。`uv sync --locked` を使い、ロックファイルのドリフトでビルドを失敗させる。
...
- [`../assets/templates/github-ci.yml`](../assets/templates/github-ci.yml) -> 通常の Python バックエンドリポジトリ向け `.github/workflows/ci.yml`。
...
`uv python install` は `.python-version` を尊重するため、ジョブはローカル開発と同じ Python パッチバージョンを使う。`uv sync --locked` は `pyproject.toml` と `uv.lock` がずれたとき CI を失敗させる。
