...
> **いつ読むか:** 既存のクラスベースまたは ORM 中心のコードベースに Kamae Python を導入するとき。
> **関連:** [`boundary-defense.md`](/docs/kamae/python/references/boundary-defense/)、[`orm-adapters.md`](/docs/kamae/python/references/orm-adapters/)、[`aggregates.md`](/docs/kamae/python/references/aggregates/)。

Kamae Python は到達状態を記述する。既存のクラスベースサービス、blob モデル、ORM 中心コードは、ビッグバン書き換えなしで**段階的**にそこへ近づけられる。

デフォルトは **Strangler Fig** パターン: 新フローと高リスク領域を先に。触れない安定レガシー経路はそのままにする。
...
サービスメソッドをポートを受け取る非同期関数にする。完全実装: [`state-transitions.md`](/docs/kamae/python/references/state-transitions/#keep-use-cases-thin)。
...
`mapped_column` エンティティ、行 DTO、`domain_from_row_dto` マッパー、Django `select_for_update` 書き込みパターンは [`orm-adapters.md`](/docs/kamae/python/references/orm-adapters/) を読む。
...
- 可観測性が [`loggable-identifiers.md`](/docs/kamae/python/references/loggable-identifiers/) に従う

最初に移行するワークフローの選択は [`aggregates.md`](/docs/kamae/python/references/aggregates/) と [`application-wiring.md`](/docs/kamae/python/references/application-wiring/) を読む。
