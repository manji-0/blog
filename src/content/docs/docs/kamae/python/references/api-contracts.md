---
title: "API 契約と docstring"
sidebar:
  order: 10
---

> **いつ読むか:** 公開ドメイン API、リポジトリプロトコル、遷移関数、DTO 変換、イベントスキーマ、安全なラッパーを文書化するとき。
> **関連:** [`domain-modeling.md`](/docs/kamae/python/references/domain-modeling/)、[`state-transitions.md`](/docs/kamae/python/references/state-transitions/)、[`unsafe-boundaries.md`](/docs/kamae/python/references/unsafe-boundaries/)、[`persistence-events.md`](/docs/kamae/python/references/persistence-events/)。

## ナレーションではなくドメイン契約を文書化する

公開ドメイン API は、呼び出し側が依存してよいものを説明すべきである: 不変条件、構築経路、状態遷移、エラー、副作用、トランザクション期待、冪等性、マスキング、安全でない/ネイティブ境界契約。
...
- イベントと DTO のドキュメントでは方針本文をコピーせず、[`persistence-events.md`](/docs/kamae/python/references/persistence-events/) と [`boundary-defense.md`](/docs/kamae/python/references/boundary-defense/) へリンクする。
