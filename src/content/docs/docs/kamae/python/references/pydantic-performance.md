---
title: "Pydantic のパフォーマンスと検証コスト"
sidebar:
  order: 10
---

> **いつ読むか:** 大きなモデル、高頻度エンドポイント、`model_construct` のトレードオフ、msgspec 風境界シリアライザーで検証オーバーヘッドが問題になるとき。
> **関連:** [`domain-modeling.md`](/docs/kamae/python/references/domain-modeling/)、[`boundary-defense.md`](/docs/kamae/python/references/boundary-defense/)、[`unsafe-boundaries.md`](/docs/kamae/python/references/unsafe-boundaries/)。
...
| ロギング / メトリクス | 大きな状態への `model_dump` | 識別子と `kind` のみログ。[`loggable-identifiers.md`](/docs/kamae/python/references/loggable-identifiers/) を参照 |
| テスト | 同一フィクスチャの再検証 | コンストラクタまたはモジュールごとに 1 回キャッシュしたアダプターでフィクスチャ構築 |

`TypeAdapter` インスタンスはモジュールレベル定数であるべきだ。Pydantic はバリデータをキャッシュする。リクエストごとに新しいアダプターを構築するとスキーマコンパイル作業を繰り返す。
...
外部 HTTP、キュー、ファイル入力で検証をスキップするために `model_construct` を使わない。完全な方針は [`boundary-defense.md`](/docs/kamae/python/references/boundary-defense/) と [`unsafe-boundaries.md`](/docs/kamae/python/references/unsafe-boundaries/) を読む。
...
3. CPU バウンドバッチは `asyncio.to_thread` またはワーカープールへオフロードを検討。[`concurrency.md`](/docs/kamae/python/references/concurrency/) を参照。
...
3. **プロセス内専用ヘルパーには dataclass を優先する。** 選択表は [`domain-modeling.md`](/docs/kamae/python/references/domain-modeling/) を参照。明示的マッパーなしに同じ概念を Pydantic と dataclass の両方で重複しない。
4. **I/O を行うバリデータを避ける。** `@field_validator` と `@model_validator` はすべての構築で実行される。高コストチェックは明示的依存を持つユースケースまたはインフラアダプターに属する。
...
2. ボトルネックが検証であることを確認する。N+1 クエリ、イベントループ上の同期 I/O、ロギングでの oversized `model_dump` ではない。
3. まず狭い DTO と `model_construct` マッパーパターンを適用する。
4. その後にだけ、Pydantic ドメインモデルを保ちつつ境界でより速いシリアライザーを検討する。

CPU バウンド検証または変換を asyncio イベントループから外すべきときは [`concurrency.md`](/docs/kamae/python/references/concurrency/) を読む。
