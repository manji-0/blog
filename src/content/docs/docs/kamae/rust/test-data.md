...
> **関連:** [`state-transitions.md`](/docs/kamae/rust/state-transitions/)、[`logging-metrics.md`](/docs/kamae/rust/logging-metrics/)、[`property-based-tests.md`](/docs/kamae/rust/property-based-tests/)。
...
helper は `tests/support/`、`#[cfg(test)] mod test_support`、crate ローカル `mod tests` で共有。fake port パターンは [`dev-environment.md`](/docs/kamae/rust/dev-environment/#fake-ports-and-test-fixtures) 参照。
...
識別子ポリシーは [`logging-metrics.md`](/docs/kamae/rust/logging-metrics/) の tier ルールを assert:
...
純粋ユースケースは fake repository、トランザクションと制約は adapter/integration。ドメインとユースケースは Docker 不要。container は infrastructure crate 用（[`dev-environment.md`](/docs/kamae/rust/dev-environment/#test-layers) 参照）。
...
- state machine 遷移法則（[`property-based-tests.md`](/docs/kamae/rust/property-based-tests/#model-state-machines-as-strategies) 参照）
- 金額算術、単位変換、タイムスタンプ境界
...
制約フィールドは public constructor から組んだ明示 strategy を優先。shrink、regression、CI 予算は [`property-based-tests.md`](/docs/kamae/rust/property-based-tests/) 参照。
...
PR 前に [`quality-gates.md`](/docs/kamae/rust/quality-gates/) のテストコマンドを実行。
...
[`property-based-tests.md`](/docs/kamae/rust/property-based-tests/) も照合する。値オブジェクト検証、往復、遷移法則、冪等性に例表カバレッジがなく、ジェネレータが公開コンストラクタを使えるときはプロパティテストを提案する。
