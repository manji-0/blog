...
> **関連:** [`state-transitions.md`](/docs/kamae/rust/state-transitions/)、[`boundary-defense.md`](/docs/kamae/rust/boundary-defense/)、[`persistence-events.md`](/docs/kamae/rust/persistence-events/)、[`property-based-tests.md`](/docs/kamae/rust/property-based-tests/)。
...
トランザクションスコープ・バージョニング・集約横断の調整は [`persistence-events.md`](/docs/kamae/rust/persistence-events/) を参照する。
...
[`boundary-defense.md`](/docs/kamae/rust/boundary-defense/) を参照する。
...
各状態が異なるフィールドを持ち遷移が主 API なら、別の状態構造体を優先する（[`state-transitions.md`](/docs/kamae/rust/state-transitions/)）。typestate と状態構造体は併用できる。例: `ExpenseReport<Submitted>` が `SubmittedReport` を包む。
...
secret や PII を含み、ログで map key に誤用しうる型に `Hash` / `Eq` derive しない。[`pii-protection.md`](/docs/kamae/rust/pii-protection/) を参照する。
...
| スタック | モデリングパターン |
| --- | --- |
| `nutype` + `thiserror` | 生成 guard 付き検証 newtype（[`crate-guides/nutype.md`](/docs/kamae/rust/crate-guides/nutype/)） |
| `rust_decimal` + newtypes | checked 算術の `Money`、`TaxRate` |
| `serde(try_from)` + newtypes | JSON 境界のリーフ value object（[`boundary-defense.md`](/docs/kamae/rust/boundary-defense/)） |
| `proptest` + builders | primitive に `Arbitrary` のあと `try_new`（[`property-based-tests.md`](/docs/kamae/rust/property-based-tests/)） |
