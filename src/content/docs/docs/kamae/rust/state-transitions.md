...
> **関連:** [`domain-modeling.md`](/docs/kamae/rust/domain-modeling/)、[`persistence-events.md`](/docs/kamae/rust/persistence-events/)、[`persistence-events.md`](/docs/kamae/rust/persistence-events/)。
...
ユースケースが分解し、[`persistence-events.md`](/docs/kamae/rust/persistence-events/) 経由で persist し event を publish する。遷移メソッド内の global buffer に event を積まない。
...
- **Typestate phantom marker**: フェーズ間で同じデータ形状だが操作が異なる; [`domain-modeling.md`](/docs/kamae/rust/domain-modeling/#typestate-with-phantom-types) 参照
- **集約トランザクション**: ユースケースが version 付き集約を load、純粋遷移、原子的 save; [`persistence-events.md`](/docs/kamae/rust/persistence-events/) 参照
...
バージョン付き保存とトランザクション境界の期待について [`persistence-events.md`](/docs/kamae/rust/persistence-events/) も照合する。
