...
> **関連:** [`boundary-defense.md`](/docs/kamae/rust/boundary-defense/)、[`stream-continuous-queries.md`](/docs/kamae/rust/stream-continuous-queries/)、[`application-wiring.md`](/docs/kamae/rust/application-wiring/)。
...
read model、統合、オペレータが変更フィードを購読するとき、ユースケース内 ad-hoc poll ではなく `futures::Stream` port で永続 event または outbox 行を公開。[`stream-continuous-queries.md`](/docs/kamae/rust/stream-continuous-queries/) で backpressure、checkpoint、projection idempotency。
...
`ConcurrentModification` を型付きユースケースエラーとして公開し、HTTP が 409、queue consumer が fresh load でリトライできるようにする。[`persistence-events.md`](/docs/kamae/rust/persistence-events/#optimistic-concurrency-is-the-default) 参照。
...
persistence adapter も DTO -> ドメイン変換に従う。[`boundary-defense.md`](/docs/kamae/rust/boundary-defense/#database-rows-sqlxfromrow) 参照。破損またはレガシー行は adapter で `RepositoryError::CorruptRow` とし、無効 domain state を出さない。
...
[テストデータ](/docs/kamae/rust/test-data/) も参照。冪等キーや重複排除レコードなしに同一遷移を二重適用しうるコマンドハンドラやコンシューマを指摘する。
