...
> **関連:** [`boundary-defense.md`](/docs/kamae/rust/boundary-defense/)、[`persistence-events.md`](/docs/kamae/rust/persistence-events/)、[`stream-continuous-queries.md`](/docs/kamae/rust/stream-continuous-queries/)。
...
可能なら idempotency key を副作用と同じ store に永続化。outbox 公開は [`persistence-events.md`](/docs/kamae/rust/persistence-events/) に合わせる。
...
分散 trace をドメイン監査ログと混同しない。耐久性が必要なら outbox 経由でビジネス event を永続化（[`logging-metrics.md`](/docs/kamae/rust/logging-metrics/) 参照）。
...
`Cargo.toml` に `tonic`、`prost`、`lapin`、`rdkafka`、`aws-sdk-sqs` などがあるとき、このガイドを [`boundary-defense.md`](/docs/kamae/rust/boundary-defense/) と consumer/projection 向け [`stream-continuous-queries.md`](/docs/kamae/rust/stream-continuous-queries/) と一緒に読み込む。
...
[`persistence-events.md`](/docs/kamae/rust/persistence-events/) も照合する。冪等キーや重複排除ストレージなしに副作用を適用するコンシューマを指摘する。
