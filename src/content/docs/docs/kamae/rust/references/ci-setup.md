---
title: "Rust CI セットアップ"
sidebar:
  order: 10
---

> **対象読者:** スキルを自リポジトリにインストールするプロジェクト。`kamae-rs` スキルリポジトリ本体は [`development-setup.md`](/docs/kamae/rust/references/development-setup/) を参照。
> **読むタイミング:** GitHub Actions、branch protection、リポジトリ検証 job の作成・更新。
> **関連:** [`quality-gates.md`](/docs/kamae/rust/references/quality-gates/)（CI が走らせるチェック）、[`local-validation.md`](/docs/kamae/rust/references/local-validation/)。

## 基本方針

CI はレビュアが依存する安全シグナルを強制する: フォーマット、lint、テスト、rustdoc、パッケージ固有検証。デフォルト pipeline は地味で速く。リスク低減が real なときだけ重いチェックを追加。

既存コマンドを先に使う。CI がなければ、変更 Rust domain コードをカバーする最小 workflow から始める。

## デフォルト GitHub Actions workflow

CI は [`quality-gates.md`](/docs/kamae/rust/references/quality-gates/) と同じチェックを実行。`rust-toolchain.toml` または `dtolnay/rust-toolchain@...` で Rust toolchain を pin し、ローカルと CI が同じ component を使う。
...
- チェック名は vague な quality ラベルではなく、守るリスクに合わせる
...
フル parity が遅すぎるなら fast path と full path を分けて文書化。[`dev-environment.md`](/docs/kamae/rust/references/dev-environment/) と [`quality-gates.md`](/docs/kamae/rust/references/quality-gates/) で toolchain、test 層、推奨ローカル check loop。
