...
- domain、use-case、transition module に `unsafe`
- FFI 戻り値が `TryFrom` なしで domain 型に変換
- `assume_init` または証明済み `write` 前に `MaybeUninit` を read
- アドレス取得後に self-referential struct を move
- redaction ポリシーなしに PII や secret が plain `*const c_char` で FFI を越える
