...
1 つの state だけが遷移できるとき、その特定 state を受け取る。広い enum ではない。
...
ユースケースが分解し、[`persistence-events.md`](/docs/kamae/rust/references/persistence-events/) 経由で persist し event を publish する。遷移メソッド内の global buffer に event を積まない。
...
- **Typestate phantom marker**: フェーズ間で同じデータ形状だが操作が異なる; [`domain-modeling.md`](/docs/kamae/rust/references/domain-modeling/#typestate-with-phantom-types) 参照
- **集約トランザクション**: ユースケースが version 付き集約を load、純粋遷移、原子的 save; [`aggregate-transactions.md`](/docs/kamae/rust/references/aggregate-transactions/) 参照

## レビューシグナル

次をフラグ:

- 遷移が `&mut TaxiRequest` を取り `status: String` を設定
- 型システムで強制できる前提を `panic!` や `unwrap` で符号化
- 遷移コード内 global または `static` buffer へ event 追加
- テスト seam なしで遷移内に `OccurredAt::now()` 固定
- move 意味論なしに同一ソース state を 2 遷移に使用
