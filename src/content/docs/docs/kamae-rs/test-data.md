...
> **関連:** [状態遷移](/docs/kamae-rs/state-transitions/)、[ロギングとメトリクス](/docs/kamae-rs/logging-metrics/)、[プロパティベーステスト](/docs/kamae-rs/property-based-tests/)。
...
純粋ユースケースは fake repository、トランザクションと制約は adapter/integration。ドメインとユースケースは Docker 不要。container は infrastructure crate 用（[開発環境](/docs/kamae-rs/dev-environment/#test-layers) 参照）。
