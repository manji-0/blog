...
> **対象読者:** スキルを自リポジトリにインストールするプロジェクト。`kamae-rs` スキルリポジトリ本体は [`development-setup.md`](/docs/kamae-rs/development-setup/) を参照。
> **いつ読むか:** GitHub Actions、branch protection、リポジトリ検証 job の作成・更新。
> **関連:** [`quality-gates.md`](/docs/kamae-rs/quality-gates/)（CI が走らせるチェック）、[`development-setup.md`](/docs/kamae-rs/dev-environment/)。
...
CI は [`quality-gates.md`](/docs/kamae-rs/quality-gates/) と同じチェックを実行する。`rust-toolchain.toml` または `dtolnay/rust-toolchain@...` で Rust toolchain を pin し、ローカルと CI が同じ component を使う。
...
フル parity が遅すぎるなら fast path と full path を分けて文書化。[`dev-environment.md`](/docs/kamae-rs/dev-environment/) と [`quality-gates.md`](/docs/kamae-rs/quality-gates/) で toolchain、test 層、推奨ローカル check loop。
...
触ったクレート向けにパッケージ検証、フォーマット、lint、テストを走らせる短いローカルコマンド一覧またはスクリプトを提案する。正規コマンドは [`quality-gates.md`](/docs/kamae-rs/quality-gates/)、推奨の高速パスとフル pre-push ループは [`dev-environment.md`](/docs/kamae-rs/dev-environment/) を照合する。
