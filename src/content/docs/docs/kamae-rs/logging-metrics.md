...
遷移の記録はユースケース境界で行う（[状態遷移](/docs/kamae-rs/state-transitions/)）。マスキングとID分類は [PII 保護](/docs/kamae-rs/pii-protection/)、エラーの一度きりの記録は [エラーハンドリング](/docs/kamae-rs/error-handling/) と整合させる。
...
logは長寿命で広くアクセス可能： 公開境界として扱う。[PII 保護](/docs/kamae-rs/pii-protection/) のルールに従う。
...
error enum設計は [エラーハンドリング](/docs/kamae-rs/error-handling/) と照合。ユースケースがricher domain contextで同一失敗をlog済みならrepository adapterで重複logしない。
