...
> **関連:** [`domain-modeling.md`](/docs/kamae-rs/domain-modeling/)、[`error-handling.md`](/docs/kamae-rs/error-handling/)、[`crate-guides.md#serde`](/docs/kamae-rs/crate-guides/#serde)。
...
不変条件を持つ単一フィールドのリーフ型では、通常コードと同じコンストラクタにデシリアライズを委譲する。[`crate-guides.md#serde`](/docs/kamae-rs/crate-guides/#serde) も参照。
...
| `garde` + `serde` + axum | `TryFrom` の前または内部で DTO を `garde` 検証; [`crate-guides.md#garde`](/docs/kamae-rs/crate-guides/#garde) 参照 |
...
レビューでは、ハンドラが `String` ID をユースケースへ直接渡すこと、ドメイン struct の `Deserialize` / `FromRow` / 無制限 `Serialize` derive、インバウンド DTO の意味を変える `default`、認証コンテキストと照合しない tenant / actor ID、ドメイン遷移に到達する `serde_json::Value` や `prost` 型を指摘する。
...
値がアダプタ層に留まる生 DTO / リードモデル構築、検証付き `TryFrom` / コンストラクタ経路内の直接ドメイン構築には指摘しない。
