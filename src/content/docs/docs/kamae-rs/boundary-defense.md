...
状態とnewtypeの設計は [ドメインモデリング](/docs/kamae-rs/domain-modeling/)、エラーの返し方は [エラーハンドリング](/docs/kamae-rs/error-handling/)、serdeの使い分けは [クレートガイド（serde）](/docs/kamae-rs/crate-guides/#serde) を参照する。
...
非空文字列、有効なID、正の金額、範囲、クロスフィールドルールなどのドメイン不変条件を `Deserialize` だけに頼るコードを指摘する。

### 外部境界はすべて DTO → ドメインで変換されているか — High

HTTPハンドラ、キューコンシューマ、DB行マッパ、ファイル / 設定 / 環境リーダ、CLIパーサが、検証付き変換なしに生データをドメインロジックへ渡す箇所を指摘する。

値がアダプタ層に留まる生DTO / リードモデル構築、検証付き `TryFrom` / コンストラクタ経路での直接ドメイン構築には指摘しない。
...
欠落や綴り違いの入力がビジネス意味を変えうる受信DTOで、広い `Default`、オプションフィールド、寛容な未知フィールド処理を指摘する。互換性が不要なら明示的な既定値と `deny_unknown_fields` を優先する。

### ドメインエンティティは外部形式向けに過剰に derive されていないか — Medium

不変条件やマスキングを守るために別DTO / 行で十分なのに、ドメインエンティティに `Deserialize`、`Serialize`、`FromRow` がある場合は指摘する。

意図的なリードモデル、プロジェクション、レスポンス専用DTOの `Serialize` には指摘しない。

### 検証済みリーフのデシリアライズと集約パースは区別されているか — Medium

クロスフィールドルールをDTOと `TryFrom` で強制できるのに、集約、コマンド、複数フィールドエンティティに `Deserialize` がある場合は指摘する。通常コードと同じコンストラクタへ委譲するリーフ値オブジェクト（ID、メール、スラッグ）の `#[serde(try_from = "...")]` には指摘しない。
