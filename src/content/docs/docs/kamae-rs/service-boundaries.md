...
可能なら idempotency key を副作用と同じ store に永続化。outbox 公開は [永続化、集約、イベント](/docs/kamae-rs/persistence-events/) に合わせる。
...
ユースケースが失敗をリトライ可能とするか補償とするか決める。adapter がポリシーを実行する。
...
分散 trace をドメイン監査ログと混同しない。耐久性が必要なら outbox 経由でビジネス event を永続化（[ロギングとメトリクス](/docs/kamae-rs/logging-metrics/) 参照）。
...
`Cargo.toml` に `tonic`、`prost`、`lapin`、`rdkafka`、`aws-sdk-sqs` などがあるとき、このガイドを [境界防御](/docs/kamae-rs/boundary-defense/) と consumer/projection 向け [ストリームと継続クエリ](/docs/kamae-rs/stream-continuous-queries/) と一緒に読み込む。
...
[永続化、集約、イベント](/docs/kamae-rs/persistence-events/) も照合する。冪等キーや重複排除ストレージなしに副作用を適用するコンシューマを指摘する。
