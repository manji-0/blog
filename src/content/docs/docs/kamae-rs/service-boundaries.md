...
可能ならidempotency keyを副作用と同じstoreに永続化。outbox公開は [永続化、集約、イベント](/docs/kamae-rs/persistence-events/) に合わせる。
...
分散traceをドメイン監査ログと混同しない。耐久性が必要ならoutbox経由でビジネスeventを永続化（[ロギングとメトリクス](/docs/kamae-rs/logging-metrics/) 参照）。
...
`Cargo.toml` に `tonic`、`prost`、`lapin`、`rdkafka`、`aws-sdk-sqs` などがあるとき、このガイドを [境界防御](/docs/kamae-rs/boundary-defense/) とconsumer/projection向け [ストリームと継続クエリ](/docs/kamae-rs/stream-continuous-queries/) と一緒に読み込む。
